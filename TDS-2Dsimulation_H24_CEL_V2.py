...
for temps in range(5000):
    ...
    for ij in hitlist:
        ...
        # if i == atom_id :
        #     vxParticule.append(p[i].x/mass)
        # elif j == atom_id :
        #     vxParticule.append(p[j].x / mass)
        ...
        if (i or j) == atom_id:
            ...
            # vxParticule.append(p[atom_id].x/mass)
            reset = True
    if reset:
        posParticule = []
        tickParticule = 0
...
# v_particule_normalized = v_particule/max(v_particule)
# v_particule_2_normalized = v_particule**2/max(v_particule**2)
# vxParticule_normalized = np.array(vxParticule)**2/max(np.array(vxParticule)**2)
...
ax[0].hist(v_norm_normalized, bins=bin_nb, alpha=0.5, label=r"$||\vec{v}||$"f"\n Moyenne = {param_list[0][0]}\n"
                                               f" Mode = {param_list[0][1]}\n Médiane = {param_list[0][2]}"
            f"\n RMS = {param_list[0][3]}" , color="b")
ax[0].legend(loc="upper right")
ax[1].hist(v_2_normalized, bins=bin_nb, alpha=0.5, label=r"$v^2$"f"\n Moyenne = {param_list[1][0]}\n"
                                               f" Mode = {param_list[1][1]}\n Médiane = {param_list[1][2]}"
            f"\n RMS = {param_list[1][3]}", color="orange")
ax[1].legend(loc="upper right")
ax[2].hist(v_x_2_normalized, bins=bin_nb, alpha=0.5, label=r"$v_x^2$"f"\n Moyenne = {param_list[2][0]}\n"
                                               f" Mode = {param_list[2][1]}\n Médiane = {param_list[2][2]}"
            f"\n RMS = {param_list[2][3]}", color="g")
ax[2].legend(loc="upper right")
# plt.xlabel("Amplitude normalisée [-]")
# plt.ylabel("Fréquence [-]")
fig.text(0.5, 0.04, 'Amplitude normalisée [-]', ha='center')
fig.text(0.04, 0.5, 'Fréquence [-]', va='center', rotation='vertical')
...
# popt, pcov = curve_fit(lambda t, a, b: a * np.exp(-1/b*t**2), v_x_normalized)
# a = popt[0]
# b = popt[1]
# c = popt[2]
# x_fitted = np.linspace(np.min(v_x_normalized), np.max(v_x_normalized), 100)
# y_fitted = a * np.exp(-1/b*x_fitted**2)
...
plt.title(r"Distribution de $v_x$ pour une particule")
plt.xlabel('Amplitude normalisée [-]')
plt.ylabel('Fréquence [-]')
plt.legend()
figManager = plt.get_current_fig_manager()
...
plt.plot(pdf_x, pdf_y, '--', color="k", label="Ajustement de courbe gaussien")
# plt.plot(x_fitted, y_fitted, color="red")
plt.title(r"Distribution de $v_x$ à un instant pour toutes les particules")
plt.xlabel('Amplitude normalisée [-]')
plt.ylabel('Fréquence [-]')
plt.legend()
figManager = plt.get_current_fig_manager()
...
# plt.hist(v_norm_normalized, alpha=0.5, bins=bin_nb, range=bin_range)
# plt.hist(v_2_normalized, alpha=0.5, bins=bin_nb, range=bin_range)
# plt.hist(v_x_2_normalized, alpha=0.5, bins=bin_nb, range=bin_range)
# # plt.hist(v_particule**2, label="$v^2$", alpha=0.5)
# # plt.hist(vxParticule, label="$v_x$", alpha=0.5)
# plt.title("Distributions des vitesses")
# plt.xlabel('Amplitude normalisée [-]')
# plt.ylabel('Fréquence [-]')
# plt.legend([r"$||\vec{v}||$ "f"(moyenne = {param_list[0][0]}, mode = {param_list[0][1]}, médiane = {param_list[0][2]}"
#             f", RMS = {param_list[0][3]})" ,"$v^2$" f"(moyenne = {param_list[1][0]}, mode = {param_list[1][1]},"
#                                            f" médiane = {param_list[1][2]}, RMS = {param_list[1][3]})",
#             "$v_x^2$" f"(moyenne = {param_list[2][0]}, mode = {param_list[2][1]}, médiane = {param_list[2][2]},"
#                         f" RMS = {param_list[2][3]})"])
# figManager = plt.get_current_fig_manager()
# figManager.window.showMaximized()
# plt.show()
